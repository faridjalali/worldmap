...
let oceanSelection: any;
let countriesSelection: any;
...
function switchToQuiz(continent: string) {
...
async function initGlobe() {
  ...
  svg = d3.select(container as HTMLElement)
    .append("svg")
  ...
  const drag = d3.drag<SVGSVGElement, unknown>()
    ...
    .on("drag", (event) => {
      ...
      const nextRotate: [number, number] = [
  ...
  try {
    ...
    countriesSelection = g.selectAll(".country")
     ...
     .attr("d", path)
     .attr("data-continent", (d: any) => d.properties.continent)
     .style("fill", (d: any) => GLOBE_COLORS.continents[d.properties.continent] || GLOBE_COLORS.default)
...
function handleHover(e: MouseEvent, d: any) {
...
function handleMouseOut(_e: MouseEvent, d: any) {
    const cont = d.properties.continent;
    if (cont) {
       d3.selectAll(`.country[data-continent='${cont}']`)
...
function handleClick(_e: MouseEvent, d: any) {
    if (hasMoved) return; 
...
function render() {
  ...
  oceanSelection.attr("d", path as any);
  countriesSelection.attr("d", path as any);
}

function startRotationLoop() {
  if (rotationTimer) rotationTimer.stop();
  rotationTimer = d3.timer((_elapsed) => {
    if (isDragging) return;
